...
io.on("connection", (socket) => {
  ...
  socket.on("join-game", (data) => {
    ...
    let role = requested;
    if (role === "santiago" && !game.players.santiago) game.players.santiago = socket.id;
    else if (role === "alissa" && !game.players.alissa) game.players.alissa = socket.id;
    else if (!game.players.santiago) { role = "santiago"; game.players.santiago = socket.id; }
    else if (!game.players.alissa) { role = "alissa"; game.players.alissa = socket.id; }
    else { socket.emit("error-message", "A sala já está cheia."); return; }
  ...
  socket.on("make-choice", (data) => {
    ...
    if (!role || role !== game.turn) { socket.emit("error-message", "Agora é a vez do outro personagem."); return; }
    const choiceIndex = Number(data && data.choiceIndex);
    if (!Number.isInteger(choiceIndex) || choiceIndex < 0 || choiceIndex > 1) { socket.emit("error-message", "Escolha inválida."); return; }
    const nextScene = data && typeof data.nextScene === "string" ? data.nextScene : null;
    const nextTurn = data && typeof data.nextTurn === "string" ? data.nextTurn : null;
    if (!nextScene || !["santiago", "alissa"].includes(nextTurn)) { socket.emit("error-message", "Transição inválida."); return; }
  ...
  socket.on("restart-game", () => { resetGame(); broadcast(); });
