...
const story = {
  inicio: { chapter: "CAPÍTULO 1 — A CONCHA", speaker: "Narrador", text: "A maré baixou de repente. Na areia molhada, uma concha de cristal pulsa com uma luz azul que nenhum dos dois já viu.", turn: "santiago", choices: [
  ...
  concha: { chapter: "CAPÍTULO 1 — A CONCHA", speaker: "Alissa", text: "A concha reage ao toque de Santiago. Uma linha azul se desenha na areia e aponta para a velha torre de observação.", turn: "alissa", choices: [
  ...
  torre: { chapter: "CAPÍTULO 2 — A TORRE", speaker: "Santiago", text: "Dentro da torre existe um mapa antigo. Um símbolo no centro é idêntico ao desenho da concha. No canto alguém escreveu: ‘A resposta está no farol.’", turn: "santiago", choices: [
  ...
  caminho: { chapter: "CAPÍTULO 2 — O CAMINHO", speaker: "Alissa", text: "No caminho para o farol, Alissa percebe que Santiago está inquieto. A luz da concha reflete no rosto dele.", turn: "alissa", choices: [
  ...
  farol: { chapter: "CAPÍTULO 2 — O FAROL", speaker: "Santiago", text: "No alto do farol há uma fotografia de Alissa. No verso, uma data de vinte e oito anos atrás. O silêncio entre os dois pesa mais que o vento.", turn: "santiago", choices: [
  ...
  verdade: { chapter: "CAPÍTULO 3 — A VERDADE", speaker: "Alissa", text: "Alissa finalmente conta o que escondia: sua avó falava de uma ilha que surgia apenas quando a maré revelava a concha. A ilha guardava uma memória da família.", turn: "alissa", choices: [
  ...
  ilha: { chapter: "CAPÍTULO 3 — A ILHA", speaker: "Santiago", text: "A concha abre um caminho sobre o mar. Os dois chegam a uma pequena ilha escondida pela névoa. No centro, uma pedra guarda uma inscrição: ‘Só se abre para quem escolhe ficar.’", turn: "santiago", choices: [
  ...
  coracao: { chapter: "CAPÍTULO 3 — O CORAÇÃO DA MARÉ", speaker: "Alissa", text: "A pedra se ilumina. A concha começa a desaparecer, deixando no lugar uma pequena marca brilhante na mão de Alissa. A ilha não era uma armadilha: era uma despedida.", turn: "alissa", choices: [
  ...
  final: { chapter: "EPÍLOGO — DEPOIS DA MARÉ", speaker: "Santiago", text: "De volta à praia, o céu começa a clarear. A concha se foi, mas a marca ainda brilha. Santiago olha para Alissa e sorri. O mistério terminou. O que começa agora é outra história.", turn: "santiago", choices: [
  ...
  fim: { chapter: "FIM — A MARÉ DE VIDRO", speaker: "Narrador", text: "Algumas histórias terminam quando o mistério é resolvido. Outras começam exatamente nesse momento. Santiago e Alissa deixam a praia lado a lado, sem saber o que virá depois — e, pela primeira vez, sem pressa de descobrir.", ending: true }
};
...
function showChoiceBubble(role, text, temporary = false) {
  ...
  if (temporary) {
    ...
    choiceTimer = setTimeout(() => bubble.remove(), 2600);
  }
}
...
function choose(index) {
  ...
  if (!choice) return;
  showChoiceBubble(myRole, choice.text, true);
  document.querySelectorAll(".story-choice").forEach((button) => { button.disabled = true; });
  socket.emit("make-choice", { choiceIndex: index, choiceText: choice.text, nextScene: choice.next, nextTurn: choice.nextTurn, effects: choice.effects || {} });
}
...
function render(state) {
  ...
  renderSpeech(scene);
  renderChoices(state);
  if (state.lastChoice?.text) showChoiceBubble(state.lastChoice.role, state.lastChoice.text, true);
}

socket.on("connect", () => socket.emit("join-game", { role: myRole }));
socket.on("game-state", render);
socket.on("choice-made", (choice) => showChoiceBubble(choice.role, choice.text, true));
